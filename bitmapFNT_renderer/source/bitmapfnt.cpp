...
inline u32 BitmapFNT::get_fnt_char(const char* string, u32 aa_level, u32* next_char, u16* fnt_width)
{
	//printf("CALLED\n");
	int l, n, m;
	...
	for(pass=0;pass < numPasses;pass++) {
		...
		for(c=0;(offset + c) < count && c < BITMAPFNT_MAX_CHAR_COUNT;c++) {
			...
			if (isPrintable(pszText[offset + c])) {
				// top left
				ntex_off[idx] = get_fnt_char(pszText + offset + c, 2, &deltaUTF, &gly_Width);
