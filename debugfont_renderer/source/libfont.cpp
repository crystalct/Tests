...
u8 * AddFontFromBitmapArray(u8 *font, u8 *texture, u8 first_char, u8 last_char, int w, int h, int bits_per_pixel, int byte_order)
{
    ...
    font_datas.fonts[font_datas.number_of_fonts].color_format = NV40TCL_TEX_FORMAT_FORMAT_A4R4G4B4; //TINY3D_TEX_FORMAT_A8R8G8B8;
    ...
    for(n = first_char; n <= last_char; n++) {
        ...
        if(n == first_char) {
            offset = 0;
            rsxAddressToOffset(texture, &offset);
            font_datas.fonts[font_datas.number_of_fonts].rsx_text_offset = offset;
        }

        if(n == first_char+1) {
            offset = 0;
            rsxAddressToOffset(texture, &offset);
            font_datas.fonts[font_datas.number_of_fonts].rsx_bytes_per_char = offset - font_datas.fonts[font_datas.number_of_fonts].rsx_text_offset;
        }
    ...
    void (* ttf_callback) (u8 chr, u8 * bitmap, short *w, short *h, short *y_correction))
{
    ...
    font_datas.fonts[font_datas.number_of_fonts].color_format = NV40TCL_TEX_FORMAT_FORMAT_A4R4G4B4;
    font_datas.fonts[font_datas.number_of_fonts].first_char = first_char;
    font_datas.fonts[font_datas.number_of_fonts].last_char  = last_char;
    font_datas.align = FONT_ALIGN_LEFT;
...
float DrawString(float x, float y, const char *str)
{
	...
	display_ttf_string((int)x +1, (int)y +1, str, 0x00000000 | (font_datas.color & 0x000000ff), 0, font_datas.sx, font_datas.sy+4, NULL);

	return display_ttf_string((int)x, (int)y, str, font_datas.color, font_datas.bkcolor, font_datas.sx, font_datas.sy+4, NULL);
}
