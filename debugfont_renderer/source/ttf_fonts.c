...
int display_ttf_string(int posx, int posy, const char *string, u32 color, u32 bkcolor, int sw, int sh, int (*DrawIcon_cb)(int, int, char))
{
    ...
    while(*ustring) {
        ...
        if(!(ttf_font_datas[l].flags & 1)) { 
            ...
            printf("TROVATO L: %d - flag: %d\n", l, ttf_font_datas[l].flags);
            if(f_face[0]) FT_Set_Pixel_Sizes(face[0], TTF_UX, TTF_UY);
            if(f_face[1]) FT_Set_Pixel_Sizes(face[1], TTF_UX, TTF_UY);
            if(f_face[2]) FT_Set_Pixel_Sizes(face[2], TTF_UX, TTF_UY);
            if(f_face[3]) FT_Set_Pixel_Sizes(face[3], TTF_UX, TTF_UY);
        ...
        if(ccolor) {
            ...
            printf("PRINT:(l) %d,  %f, %f, %f, %f, %f, %d \n",l, (float)(Win_X_ttf + posx), (float)(Win_Y_ttf + posy) + ((float)ttf_font_datas[l].y_start * sh) * 0.03125f,
                Z_ttf, (float)sw, (float)sh, color);
            printf("TEXture P: %p, \n", (void*)texture_mem);
            printf("Data: %p - Diif: %ld\n", (void *)ttf_font_datas[l].text, (void*)ttf_font_datas[l].text - (void*)texture_mem);
            texture_sec = (u32 *)ttf_font_datas[l].text;
