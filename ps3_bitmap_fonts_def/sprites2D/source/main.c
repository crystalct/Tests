...
void LoadTexture()
{
    ...
    u32 * texture_mem = (u32*) tiny3d_AllocTexture(16*1024*1024); // alloc 64MB of space for textures (this pointer can be global)
...
s32 main(s32 argc, const char* argv[])
{
    ...
    u32* texture_mem = (u32*)tiny3d_AllocTexture(32 * 1024 * 1024); // alloc 64MB of space for textures (this pointer can be global)

    if (!texture_mem) exit(0); // fail!
    fnt_class alpha;
    int ret;
    ret = init_fnt((gcmContextData*)tiny3d_Get_GCM_Context(), Video_Resolution.width, Video_Resolution.height, 2, texture_mem , &alpha);
    printf("ret: %d - Valore maxwidth: %d height: %d sXPos: %d - sLeftSafe: %d - sXRes: %d\n", 
        ret, alpha.fnt.maxwidth, alpha.fnt.height, alpha.sXPos, alpha.sLeftSafe, alpha.sXRes);
    if (ret)
        exit(0);
	...
	while(1) {
        ...
        u32 temp = TINY3D_BLEND_FUNC_SRC_RGB_SRC_ALPHA | TINY3D_BLEND_FUNC_SRC_ALPHA_SRC_ALPHA;
        u32 temp2 = TINY3D_BLEND_FUNC_DST_RGB_ONE_MINUS_SRC_ALPHA | TINY3D_BLEND_FUNC_DST_ALPHA_ZERO;
        u32 temp3 = TINY3D_BLEND_RGB_FUNC_ADD | TINY3D_BLEND_ALPHA_FUNC_ADD;
        // Enable alpha blending.
        tiny3d_BlendFunc(1, TINY3D_BLEND_FUNC_SRC_RGB_SRC_ALPHA, TINY3D_BLEND_FUNC_DST_RGB_ONE_MINUS_SRC_ALPHA, TINY3D_BLEND_RGB_FUNC_ADD);
